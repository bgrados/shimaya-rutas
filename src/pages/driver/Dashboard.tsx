...
interface EstadoChofer {
  ...
  excepcionEstado?: 'descansa' | 'trabaja';
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState<Stats>({
    ...
    gastoOtrosDia: 0, gastroOtrosSemana: 0, gastosHoy: 0,
  ...
  const [excepcionesHoy, setExcepcionesHoy] = useState<Record<string, { estado: string; observaciones?: string }>>({});
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const { data: excepcionesData } = await supabase
        ...
        .select('id_chofer, estado, observaciones')
        .eq('fecha', hoyStr);

      const excepcionesMap: Record<string, { estado: string; observaciones?: string }> = {};
      if (excepcionesData) {
        excepcionesData.forEach(e => {
          excepcionesMap[e.id_chofer] = { estado: e.estado, observaciones: e.observaciones };
        });
      }
      setExcepcionesHoy(excepcionesMap);
      ...
      const choferesConEstado: EstadoChofer[] = todosChoferes.map((c: any) => {
        ...
        const excepcion = excepcionesMap[c.id_usuario];
        const tieneExcepcionHoy = !!excepcion;
        const excepcionEstado = excepcion?.estado as 'descansa' | 'trabaja' | undefined;
        const descansaPorExcepcion = excepcionEstado === 'descansa';
        const trabajaPorExcepcion = excepcionEstado === 'trabaja';
        const descansaPorDiaFijo = (c.dias_descanso || []).includes(diaHoy);

        let descansaHoy = false;
        let motivo = '';

        if (tieneExcepcionHoy) {
          if (descansaPorExcepcion) {
            descansaHoy = true;
            motivo = 'Descanso (excepción)';
          } else if (trabajaPorExcepcion) {
            descansaHoy = false;
            motivo = 'Trabaja (excepción)';
          }
        } else {
          descansaHoy = descansaPorDiaFijo;
          motivo = descansaPorDiaFijo ? 'Descanso fijo' : 'Laborando';
        }
        ...
        return {
          ...
          tieneExcepcionHoy,
          excepcionEstado,
          excepcionObservacion: excepcion?.observaciones,
          descansaHoy,
          motivo,
          enRuta: false
        };
      ...
      setStats({
        ...
        gastoOtrosDia, gastroOtrosSemana: gastoOtrosSemana,
        ...
        <Card className="bg-blue-500/10 border border-blue-500/30">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-400/60">Sem: S/ {stats.gastroOtrosSemana.toFixed(2)}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-4">
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
            {estadoChoferes.map(chofer => {
              const excepcion = excepcionesHoy[chofer.id];
              const tieneExcepcion = !!excepcion;
              const descansaPorExcepcion = excepcion?.estado === 'descansa';
              const trabajaPorExcepcion = excepcion?.estado === 'trabaja';
                ...
                <div key={chofer.id} className="flex items-center justify-between p-3 bg-surface-light/20 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${chofer.enRuta ? 'bg-green-500 animate-pulse' :
                        descansaPorExcepcion ? 'bg-orange-500' :
                          chofer.descansaHoy ? 'bg-red-500' : 'bg-blue-500'
                      }`} />
                    <span className="text-white font-medium">{chofer.nombre}</span>
                    {tieneExcepcion && (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1 ${trabajaPorExcepcion ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                          'bg-orange-500/20 text-orange-400 border border-orange-500/30'
                        }`}>
                        <Calendar size={10} />
                        {trabajaPorExcepcion ? 'Trabaja (excepción)' : 'Descanso (excepción)'}
                      </span>
                    )}
                    {chofer.enRuta ? (
          ...
          <div className="mt-3 pt-3 border-t border-white/10 flex gap-4 text-xs text-text-muted">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"></span> Descanso fijo</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-orange-500"></span> Excepción</span>
          </div>
        </CardContent>
      </Card>
