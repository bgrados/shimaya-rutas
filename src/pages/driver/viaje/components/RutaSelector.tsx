...
import { Truck, PlusCircle, ChevronDown, Camera, X, Loader2 } from 'lucide-react';
import { Button } from '../../../../components/ui/Button';
import { Card, CardContent } from '../../../../components/ui/Card';
import { Input } from '../../../../components/ui/Input';
import { useOcrModule } from '../hooks/useOcrModule';
...
export function RutaSelector({
  ...
  const { procesarKilometraje, procesandoOCR, kmDetectado } = useOcrModule();
    ...
    input.onchange = async (e: any) => {
      ...
      if (file) {
        ...
        reader.onload = async (re) => {
          ...
          const detected = await procesarKilometraje(dataUrl);
          if (detected) {
            setKmInicio(detected.toString());
            setErrorOCR(null);
          } else {
            setErrorOCR('No se pudo leer el número. Intenta nuevamente.');
          }
        };
  ...
  return (
    <div className="p-4 space-y-8 max-w-lg mx-auto pb-24">
      <div className="text-center space-y-2 pt-8">
        ...
        <div className="text-xs text-yellow-400 bg-yellow-500/10 rounded-lg p-2 mt-2 font-medium">
          📸 Al tomar la foto del odómetro, <strong className="font-black">APUNTA HORIZONTALMENTE</strong> para mejor detección
        </div>
      </div>
            ...
            <div className="space-y-1">
              ...
              {!fotoKmInicio ? (
                ...
                <div className="relative group">
                  <img src={fotoKmInicio} className="w-full h-32 object-cover rounded-xl border-2 border-primary/50" alt="Km inicio" />
                  <button
                    onClick={() => { setFotoKmInicio(null); setErrorOCR(null); }}
