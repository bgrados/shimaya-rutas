...
import { formatPeru, formatGroupDate, formatGroupDatePdf, getStartOfCurrentWeek, getEndOfCurrentWeek, formatFriendlyDate, nowPeru, calcularDuracionMinutos, formatTimeOnly } from '../../../lib/timezone';
    ...
    const rows = rutas.map(r => {
      ...
      const paradas = bits.map((b: any, i: number) => {
        ...
        return `<tr>
          ...
          <td style="padding:5px 8px;border-bottom:1px solid #f1f5f9;color:#475569;">${b.hora_salida ? formatTimeOnly(b.hora_salida) : '-'}</td>
          <td style="padding:5px 8px;border-bottom:1px solid #f1f5f9;color:#475569;">${b.hora_llegada ? formatTimeOnly(b.hora_llegada) : '⏳'}</td>
...
return `<div style="page-break-inside:avoid;margin-bottom:20px;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;">
        ...
        <div style="padding:8px 16px;background:#f8fafc;font-size:12px;color:#64748b;display:flex;gap:20px;flex-wrap:wrap;border-bottom:1px solid #e2e8f0;">
          ${r.hora_salida_planta ? `<span>🕐 Salida planta: <strong>${formatTimeOnly(r.hora_salida_planta)}</strong></span>` : ''}
          ${r.horaLlegadaReal ? `<span>🏁 Llegada planta: <strong>${formatTimeOnly(r.horaLlegadaReal)}</strong></span>` : ''}
                            ...
                            {ruta.hora_salida_planta && (
                              <span className="text-xs text-text-muted flex items-center gap-1">
                                🕐 <span className="cursor-pointer hover:text-white" onClick={() => iniciarEdicionSalida(ruta)}>
                                  {formatTimeOnly(ruta.hora_salida_planta)}
                                </span>
                              </span>
                            )}
                          </>
                            ...
                            {bits.map((b: any, i: number) => {
                              ...
                              return (
                                <tr key={b.id_bitacora} className="border-b border-white/5 hover:bg-white/5">
                                  ...
                                  <td className="px-4 py-2 text-text-muted">{b.hora_salida ? formatTimeOnly(b.hora_salida) : '-'}</td>
                                  <td className="px-4 py-2 text-text-muted">{b.hora_llegada ? formatTimeOnly(b.hora_llegada) : <span className="text-blue-400 animate-pulse">En camino</span>}</td>
