import { Fuel, Download, Trash2, Image } from 'lucide-react';
import { Card, CardContent } from '../../../../components/ui/Card';
import { Button } from '../../../../components/ui/Button';
import { formatFriendlyDate } from '../../../../lib/timezone';

interface CombustibleTableProps {
  data: any[];
  loading: boolean;
...
export function CombustibleTable({
  data,
  loading,
  onDelete,
  onViewImage,
  onExport
}: CombustibleTableProps) {
  if (loading) {
    return <div className="text-white italic animate-pulse text-center py-16">Cargando datos de combustible...</div>;
  }
  ...
  if (data.length === 0) {
    return (
      <div className="text-center py-16 bg-surface border border-dashed border-surface-light rounded-2xl">
        ...
        <p className="text-text-muted italic">No hay registros de combustible en este período.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter">Registros de Combustible</h2>
            ...
            <CardContent className="p-5 space-y-4">
              ...
              <div className="space-y-2 border-t border-white/5 pt-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-white font-mono">{item.placa || 'N/A'}</span>
                </div>
                {item.km_actual && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-muted">Kilometraje:</span>
                    <span className="text-primary font-bold">{item.km_actual} KM</span>
                  </div>
                )}
                {item.galones && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-muted">Galones:</span>
                    <span className="text-white font-bold">{item.galones} gl</span>
                  </div>
                )}
              </div>
