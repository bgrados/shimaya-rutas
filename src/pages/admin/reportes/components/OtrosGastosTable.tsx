import { FileDown, Download, Trash2, Image } from 'lucide-react';
import { Card, CardContent } from '../../../ui/Card';
import { Button } from '../../../ui/Button';
import { formatFriendlyDate } from '../../../../lib/timezone';

interface OtrosGastosTableProps {
  data: any[];
  loading: boolean;
...
export function OtrosGastosTable({
  data,
  loading,
  onDelete,
  onViewImage,
  onExport
}: OtrosGastosTableProps) {
  if (loading) {
    return <div className="text-white italic animate-pulse text-center py-16">Cargando otros gastos...</div>;
  }

  if (data.length === 0) {
    return (
      <div className="text-center py-16 bg-surface border border-dashed border-surface-light rounded-2xl">
        <FileDown size={40} className="mx-auto mb-3 text-text-muted opacity-30" />
        <p className="text-text-muted italic">No hay otros gastos registrados en este período.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter">Otros Gastos</h2>
            ...
            <CardContent className="p-5 space-y-4">
              ...
              <div className="space-y-2 border-t border-white/5 pt-3">
                ...
                <div className="flex flex-col text-sm">
                  ...
                  <span className="text-white italic mt-1">{item.descripcion || 'Sin descripción'}</span>
                </div>
              </div>
